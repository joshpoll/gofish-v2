...
export const maxOfMaybes = (xs: (number | undefined)[]) =>
  xs.every((x) => x === undefined) ? undefined : Math.max(...(xs.filter((x) => x !== undefined) as number[]));
...
export const minOfMaybes = (xs: (number | undefined)[]) =>
  xs.every((x) => x === undefined) ? undefined : Math.min(...(xs.filter((x) => x !== undefined) as number[]));

export const maybe = <T, U>(x: T | undefined, f: (x: T) => U): U | undefined => (x !== undefined ? f(x) : undefined);
